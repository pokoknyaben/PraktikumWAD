...
if (isset($_POST['tinggi_badan']) && isset($_POST['berat_badan'])){
    $tinggigw = $_POST['tinggi_badan'];
    $bbgw = $_POST['berat_badan'];
...
if (empty($error)) {
    $meteran = $tinggigw/100;
    $hasilbmi = $bbgw/($meteran**2);
    if ($hasilbmi <= 18.4){
        $status = "Underweigh";
    }elseif($hasilbmi > 18.5){
        $status = "Normal";
    }elseif($hasilbmi > 24.9){
        $status = "Overweight";
    }else{
        $status = "Obese";
    }    
        ...
        <div class="col-md-4 p-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- silakan taruh code kalian di bawah -->
                    <?php 
                    if (isset($hasilbmi) && isset($status)) {
                        echo "Hasil BMI : " .$hasilbmi;
                        echo "<br/>";
                        echo "Status : ". $status;
                    }
                    ?>
